...
def check_configuration():
    ...
    if DASHSCOPE_API_KEY == "your_api_key_here" or not DASHSCOPE_API_KEY:
        issues.append("❌ 未配置API密钥，请创建config.py文件并填入正确的API密钥")
    
    if not os.path.exists(os.path.dirname(OUTPUT_FOLDER)):
        issues.append(f"❌ 输出文件夹路径不存在：{os.path.dirname(OUTPUT_FOLDER)}")
    
    if not CONFIG_LOADED:
        issues.append("⚠️ 未找到配置文件config.py，正在使用默认配置")
        issues.append("💡 建议复制config_template.py为config.py并修改配置")
...
def main():
    ...
    if config_issues:
        ...
        # 如果API密钥未配置，直接退出
        if any("API密钥" in issue for issue in config_issues):
            input("请配置API密钥后重新运行程序，按回车键退出...")
            return
